...
function getGradientStyle(colors) {
  ...
  const colorsWithPercentage = colors.map((color, index) => {
    return `${color} ${Math.round(100 * index / (colorsCount - 1))}%`
  })
...
export function ChangeColorsWindow(props) {
  return (
    <Window>
      <h1 className="h1">CHANGE COLORS</h1>
      ...
      <BackButton onClick={props.onClick}>
        <img src="https://img.icons8.com/ios-glyphs/30/ffffff/circled-left.png" alt='back-button'/>
      </BackButton>
    </Window>
  )
}
